...
describe('equal', () => {
  ...
  it('should handle circular references in Maps (key and value) and return true', () => {
    ...
    valObj1.ref = map3;
    valObj2.ref = map4;
    expect(equal(map3, map4)).toBe(true);
  });
  ...
  it('should handle circular references in Sets and return true', () => {
    ...
    obj1.ref = set3;
    obj2.ref = set4;
    expect(equal(set3, set4)).toBe(true);
  });
  ...
  it('should return false for objects with different constructors', () => {
    ...
    expect(equal(instanceA, instanceB)).toBe(false);
  });
  ...
  it('should handle objects with custom valueOf methods', () => {
    ...
    expect(equal(obj3, obj4)).toBe(false);
  });
  ...
  it('should return false for an empty Map and a non-empty Map', () => {
    expect(equal(new Map(), new Map([['a', 1]]))).toBe(false);
    expect(equal(new Map([['a', 1]]), new Map())).toBe(false);
  });

  it('should return false for an empty Set and a non-empty Set', () => {
    expect(equal(new Set(), new Set([1]))).toBe(false);
    expect(equal(new Set([1]), new Set())).toBe(false);
  });
  ...
  it('should return false for objects with different numbers of keys', () => {
    ...
    const obj2 = { a: 1, b: 2 };
    expect(equal(obj1, obj2)).toBe(false);
    expect(equal(obj2, obj1)).toBe(false);
  });
