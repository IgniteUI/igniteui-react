# Overview  
Ignite UI for React accepts contributions, as long as they follow the guidelines explained below. When contributing you would have to follow these steps:
...
[General Naming and Coding Guidelines](../../../wiki/General-Naming-and-Coding-Guidelines-for-Ignite-UI-for-React)  
-->

# Workflow
When working on an issue for the Ignite UI for React repository, you need to be aware of and to follow a correct status workflow. We have created a number of status labels in order to communicate well what the current status of a single issue/pull request is. The statuses are as follows:
...
5. `status: third-party-issue` this is the status of an issue that has been reviewed, has been determined to be an issue, but the root case is not in the Ignite UI for React code. Example would be browser specific bugs caused by the particular browser's rendering or JavaScript engines, or an issue with the React framework. Remove other statuses and place only this one if you're the one performing the investigation.
...
<!--- ## Localization (i18n) - applicable to components' string resources -> Pending Localization Implementation
...
https://github.com/IgniteUI/igniteui-react-i18n 

**NOTE** The localization repo has been moved to live inside the `igniteui-react` repository under `./projects/igniteui-react-i18n`
...
A npm package should be published each time we release new version of Ignite UI for React. Its version should correspond to the version of the igniteui-react npm package.
One could localize an application by importing the corresponding localized resource strings from the localization package (`igniteui-react-i18n`) and use the methods described in the previous bullet to localize the whole application or part of it.

**Example:**

Inside app.module you can perform:
```ts
import { IgxResouceStringsJA } from ‘igniteui-react-i18n’;
...
2. Pull request your changes and reference the issue. Use the enforced commit message format with applicable type, scope, etc.
3. Don't forget to make the necessary status updates, as described in the workflow section.
...
1. Describe in the appropriate section in the [CHANGELOG.md](https://github.com/IgniteUI/igniteui-react/blob/master/CHANGELOG.md)
2. Add a `BREAKING CHANGE:` section to the commit message body or footer. See https://www.conventionalcommits.org

## Deprecations

If your change requires deprecating a method, property or member, make sure to use the [ObsoleteAttribute](https://learn.microsoft.com/en-us/dotnet/api/system.obsoleteattribute?view=net-10.0) to indicate that.

The appropriate flow with any breaking change involving API removal is to deprecate the said piece of API for the span of at least one major version before removing it entirely.
