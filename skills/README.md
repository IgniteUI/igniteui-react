...
| [igniteui-react-migrate-grid-lite-to-premium](./igniteui-react-migrate-grid-lite-to-premium/SKILL.md) | Grid Lite lacks a needed feature — editing, selection, paging, grouping, export |
...
Every skill works without them, falling back to the type declarations in `node_modules`. Setup instructions are in [igniteui-react-components/references/MCP.md](./igniteui-react-components/references/MCP.md) and the theming skill.
...
Keep each skill's `SKILL.md` alongside its `references/` folder (when present) — the reference files are loaded on demand and the relative links between skills assume the layout is preserved.

```
.claude/skills/
  igniteui-react-components/{SKILL.md,references/}
  igniteui-react-customize-theme/{SKILL.md,references/}
  igniteui-react-optimize-bundle-size/SKILL.md
  igniteui-react-generate-from-image-design/{SKILL.md,references/}
  igniteui-react-migrate-grid-lite-to-premium/SKILL.md
```
